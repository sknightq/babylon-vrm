...
export class SpringBoneImporter {
  ...
  protected async _importSpringBoneGroupList(loader: GLTFLoader, schemaSecondaryAnimation: VRMSchema.SecondaryAnimation, colliderGroups: SpringBoneColliderGroup[]): Promise<SpringBoneGroup[]> {
      ...
      springBoneGroups.map(async vrmBoneGroup => {
        ...
        await Promise.all(
          vrmBoneGroup.bones.map(async nodeIndex => {
            ...
            const center: BABYLON.TransformNode | null = vrmBoneGroup.center! !== -1 ? loader.babylonScene.transformNodes[vrmBoneGroup.center!] : null
   ...
   */
  protected async _importColliderMeshGroups(loader: GLTFLoader, schemaSecondaryAnimation: VRMSchema.SecondaryAnimation): Promise<SpringBoneColliderGroup[]> {
  ...
  protected _createColliderMesh(radius: number, offset: BABYLON.Vector3, scene: BABYLON.Scene): SpringBoneColliderMesh {
    ...
    // more detail:
    // @see https://forum.babylonjs.com/t/dimensions-of-boundingsphere-of-sphere-mesh-dont-match/14992/4
    // @see https://gamedev.stackexchange.com/questions/159511/how-can-i-generate-the-smallest-enclosing-sphere-from-a-mesh
